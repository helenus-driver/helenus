...
/**
 * The <code>BooleanCodecProvider</code> class defines a provider suitable to
 * decode the {@link DataType#BOOLEAN} data type.
 *
...
public final class BooleanCodecProvider implements CodecProvider {
   ...
   * @author paouelle
   */
  public final static CodecProvider INSTANCE = new BooleanCodecProvider();

  /**
   * Instantiates a new <code>BooleanCodecProvider</code> object.
   *
   * @author paouelle
   */
  ...
  public <T> TypeCodec<T> codecFor(Class<T> clazz)
    ...
    if ((Boolean.class == clazz) || (Boolean.TYPE == clazz)) {
      return (TypeCodec<T>)TypeCodec.cboolean();
    }
    throw new CodecNotFoundException(
      "unsupported Helenus codec from '"
      + DataType.BOOLEAN.toCQL()
      + "' to class: "
      + clazz.getName(),
      com.datastax.driver.core.DataType.cboolean(),
      TypeToken.of(clazz)
    );
  }
}
