...
/**
 * The <code>VarCharCodecProvider</code> class defines a provider suitable to
 * decode the {@link DataType#VARCHAR} data type.
 *
 ...
 * @since 1.0
 */
public final class VarCharCodecProvider implements CodecProvider {
  /**
   * Holds the single instance of this provider.
   *
   * @author paouelle
   */
  public final static CodecProvider INSTANCE = new VarCharCodecProvider();

  /**
   * Holds a default Json codec instance.
   *
   * @author paouelle
   */
  public final static TypeCodec<JsonStructure> JSON_CODEC = new Jsr353JsonCodec();

  /**
   * Instantiates a new <code>VarCharCodecProvider</code> object.
   *
   * @author paouelle
   */
  private VarCharCodecProvider() {}
  ...
  public <T> TypeCodec<T> codecFor(Class<T> clazz)
    ...
    if (String.class.isAssignableFrom(clazz)) {
      ...
      return (TypeCodec<T>)VarCharCodecProvider.JSON_CODEC;
    }
    throw new CodecNotFoundException(
      "unsupported Helenus codec from '"
      + DataType.VARCHAR.toCQL()
      + "' to class: "
      + clazz.getName(),
