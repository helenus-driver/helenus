...
/**
 * The <code>TimeCodecProvider</code> class defines a provider suitable to
 * decode the {@link DataType#TIME} data type.
 *
 ...
 * @since 3.0
 */
public final class TimeCodecProvider implements CodecProvider {
  /**
   * Holds the single instance of this provider.
   *
   * @author paouelle
   */
  public final static CodecProvider INSTANCE = new TimeCodecProvider();

  /**
   * Instantiates a new <code>TimeCodecProvider</code> object.
   *
   * @author paouelle
   */
  private TimeCodecProvider() {}
  ...
  public <T> TypeCodec<T> codecFor(Class<T> clazz)
    ...
    if ((Long.class == clazz) || (Long.TYPE == clazz)) {
      return (TypeCodec<T>)TypeCodec.time();
    } else if (LocalTime.class.isAssignableFrom(clazz)) {
      return (TypeCodec<T>)LocalTimeCodec.instance;
    }
