...
/**
 * The <code>FloatCodecProvider</code> class defines a provider suitable to
 * decode the {@link DataType#FLOAT} data type.
 *
 ...
 * @since 1.0
 */
public final class FloatCodecProvider implements CodecProvider {
  /**
   * Holds the single instance of this provider.
   *
   * @author paouelle
   */
  public final static CodecProvider INSTANCE = new FloatCodecProvider();

  /**
   * Instantiates a new <code>FloatCodecProvider</code> object.
   *
   * @author paouelle
   */
  ...
  public <T> TypeCodec<T> codecFor(Class<T> clazz)
    ...
    if ((Float.class == clazz) || (Float.TYPE == clazz)) {
      return (TypeCodec<T>)TypeCodec.cfloat();
    }
    throw new CodecNotFoundException(
      "unsupported Helenus codec from '"
      + DataType.FLOAT.toCQL()
      + "' to class: "
      + clazz.getName(),
      com.datastax.driver.core.DataType.cfloat(),
      TypeToken.of(clazz)
    );
  }
}
