...
/**
 * The <code>InetCodecProvider</code> class defines a provider suitable to
 * decode the {@link DataType#INET} data type.
 *
 ...
 * @since 1.0
 */
public final class InetCodecProvider implements CodecProvider {
  /**
   * Holds the single instance of this provider.
   *
   * @author paouelle
   */
  public final static CodecProvider INSTANCE = new InetCodecProvider();

  /**
   * Instantiates a new <code>InetCodecProvider</code> object.
   *
   * @author paouelle
   */
  private InetCodecProvider() {}
  ...
  public <T> TypeCodec<T> codecFor(Class<T> clazz)
    ...
    if (InetAddress.class.isAssignableFrom(clazz)) {
      return (TypeCodec<T>)TypeCodec.inet();
    }
    throw new CodecNotFoundException(
      ...
      + "' to class: "
      + clazz.getName(),
      com.datastax.driver.core.DataType.inet(),
      TypeToken.of(clazz)
    );
  }
}
